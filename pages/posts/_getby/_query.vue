    ...
    <section class="query">
      <section class="post-list">
        <div class="section-title">
          <h1>
            <span
              v-if="$route.params.getby == 'category' || $route.params.getby == 'tag'"
            >{{ thisQuery.fields.name }}</span>
            <span v-else>{{ $route.params.query }}</span>
          </h1>
          <p class="subtitle">
            <span v-if="$route.params.getby == 'category'">{{ labelCategory }}</span>
            <span v-else-if="$route.params.getby == 'tag'">{{ labelTag }}</span>
            <span v-else-if="$route.params.getby == 'search'">{{ labelSearchResult }}</span>
          </p>
        </div>
        <div v-if="postsFiltered.length > 0">
          <div class="posts">
            <BoxItem
              v-for="post in orderBy(limitBy(postsFiltered, showNum), listPostsOrder, listPostsOrderDirection)"
              :key="post.sys.id"
              :post="post"
            />
          </div>
          <p
            v-if="showNum < postsFiltered.length"
            class="load-btn"
...
export default {
  head() {
    if (
      this.$route.params.getby == "category" ||
      this.$route.params.getby == "tag"
    ) {
      return {
